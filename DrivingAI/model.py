...
class DoubleNetwork():
    ...
    def __init__(self, images, variables, outputs=None, training=True):
        ...
        with tf.variable_scope("Network_A"):
            self.network_a = Network(images, variables, outputs, training, global_step=self.global_step)
        with tf.variable_scope("Network_B"):
            self.network_b = Network(images, variables, outputs, training)
        if training:
...
class Network():
    ...
    def __init__(self, input_image, input_variables, example_output, training=True, global_step=None):
        ...
        if example_output is not None:
            self.loss = tf.losses.mean_squared_error(self.output, example_output) + tf.losses.get_regularization_loss()
            adam = tf.train.AdamOptimizer(1e-5, 0.85)
            self.trainer = adam.minimize(self.loss, global_step)
